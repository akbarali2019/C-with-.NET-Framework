...
{
    public class FirestoreEndDayRecordMap
    {
        public static Dictionary<string, object> MapDataForFirestore(IEnumerable<DayEndRecord> items)
        {
            ...
            {
                var fireItem = FirestoreEndDayRecordProperty.MapEndDayRecord(item);
                var documentId = $"[{item.ItemId}_{item.FacCode}]";
